'''
This is the script only for running small citation benchmarks, i.e., cora, citeseer and pubmed
'''
...
def main(args):
    ...
    if args.model == 'SAGEMLP':
        ...
        trainer = trainers.BaseTrainer(g, model, info_dict)
    elif args.model == 'GCNTSS':
        info_dict.update({'backbone': 'GCN'})
        model = models.GCN(info_dict)
        trainer = trainers.TwoStageShufTrainer(g, model, info_dict)
    elif args.model == 'SAGETSS':
        info_dict.update({'backbone': 'SAGE'})
        model = models.SAGE(info_dict)
        trainer = trainers.TwoStageShufTrainer(g, model, info_dict)
    elif args.model == 'SAGEMLPTSS':
        info_dict.update({'backbone': 'SAGEMLP'})
        model = models.SAGEMLP(info_dict)
        trainer = trainers.TwoStageShufTrainer(g, model, info_dict)
    elif args.model == 'GATTSS':
        ...
        info_dict.update({'backbone': 'GCN'})
        model = models.GCN(info_dict)
        trainer = trainers.ProgClsShufTrainer(g, model, info_dict)
    elif args.model == 'SAGEPS':
        info_dict.update({'backbone': 'SAGE'})
        model = models.SAGE(info_dict)
        trainer = trainers.ProgClsShufTrainer(g, model, info_dict)
    elif args.model == 'SAGEMLPS':
        info_dict.update({'backbone': 'SAGEMLP'})
        model = models.SAGEMLP(info_dict)
        trainer = trainers.ProgClsShufTrainer(g, model, info_dict)
        ...
        trainer = trainers.ProgClsModeContraTrainer(g, model, info_dict, Dis=Dis)
        ...
        model = models.GCN(info_dict)
        trainer = trainers.BaseTrainerClsGTFeat(g, model, info_dict)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model", type=str, default='SGCCTRS',
                        help="model name")
    ...
    parser.add_argument("--n_epochs", type=int, default=1200,
                        help="number of training epochs")
    parser.add_argument("--n_step_epochs", type=int, default=20,
                        help="epoch gap to update the estimated label, only works for -PS models")
    ...
    parser.add_argument("--emb_hid_dim", type=int, default=256,
                        help="number of hidden gcn units")
